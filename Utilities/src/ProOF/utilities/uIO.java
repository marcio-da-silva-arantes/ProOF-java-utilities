...
public final class uIO {
    ...
    public static ArrayList<Integer> toSet(String line) {
        String[] sa = line.split(",");
    ...
    public static ArrayList<Integer> ReadSetOpl(Scanner input) {
        String S = nextCMD2(input);
        S = S.split("=")[1];
        S = S.replaceFirst(";", "");
        return ReadSetOpl(S);
    ...
    private static String replaceSetOpl(String S) {
        S = S.replace("\t", "");
        S = S.replace("{ ", "{");
        S = S.replace(" {", "{");
        S = S.replace("} ", "}");
    ...
    private static ArrayList<Integer> ReadSetOpl(String S) {
        S = replaceSetOpl(S);
        if(PRINT)System.out.println(S);
        return toSet(S);
    }
    ...
    private static String replaceMatrixOpl(String S) {
        ...
        if(PRINT)System.out.println("A = "+S);
        S = S.replaceAll("/,", "/");
        if(PRINT)System.out.println("B = "+S);
        return S;
    }
